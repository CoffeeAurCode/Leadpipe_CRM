...
- `update_shared_agents.py` — push latest config to complaint agent + shared lease agent; uses direct **HTTP PATCH** (same as `update_lease_agents.py`; rewritten off the VAPI SDK, which silently dropped `firstMessageMode` and apiRequest tool subfields). Supports `--dry-run`. **Preserves the complaint French handoff across redeploys**: passes `french_assistant_id` to `build_complaint_config` from `VAPI_COMPLAINT_FRENCH_ASSISTANT_ID` — so re-running it does NOT strip the gated handoff tool. Only patches the two ENTRY assistants; the dedicated FR complaint assistant is patched by re-running `provision_complaint_french_handoff.py`
- `provision_complaint_french_handoff.py` — complaint analog of `provision_lease_french_handoff.py`, for the single GLOBAL complaint agent. Creates the dedicated **French-only** complaint assistant (nova-3 `fr`, `FRENCH_TRANSCRIBER_CONFIG`) and wires a **gated handoff** tool onto the entry complaint assistant so French callers are transferred by `assistantId` with `contextEngineeringPlan:"all"` (continues, not restarts). **No squad, no phone-number repoint**; English path unchanged. No Supabase/DB migration (complaint agent isn't per-manager) — the FR assistant id is stored in env `VAPI_COMPLAINT_FRENCH_ASSISTANT_ID`. Re-running patches the existing FR assistant in place (id read from env). `--dry-run` previews; `--rollback` removes the handoff from the entry + deletes the FR assistant. Base python is fine (no `supabase` needed). **2026-07-02:** complaint `[System-Check Phrases]` rewritten to require the checking phrase and the lookup tool call in the SAME turn (the old "say it before the tool runs" wording made the FR assistant speak the filler and end its turn without calling the tool → dead air → `silence-timed-out`), and digits-only flat numbers are now explicitly accepted (letter-prefix question asked at most once). See `docs/diagnoses/complaint_agent_french_inbound_and_tool_call_dead_air_2026-07-02.md`
- `provision_complaint_language_routing.py` — per-tenant language routing for the complaint agent (migration 032): creates/patches the **English-locked** complaint assistant (`VAPI_COMPLAINT_ENGLISH_ASSISTANT_ID`); `--route-number` repoints the complaint number to `server.url = /voice/inbound-router` (clears static assistantId) — run ONLY after the backend with that route is deployed; `--rollback` restores the static entry binding. `--dry-run` supported; base python fine
- `update_lease_agents.py` — push latest `vapi_agent_config.py` to **all active per-manager lease assistants** (iterates `manager_vapi_config` rows); fetches each manager's name from `manager_profiles` and passes to `build_lease_config`; uses direct **HTTP PATCH** to `api.vapi.ai/assistant/{id}` (not the VAPI SDK — SDK silently drops `firstMessageMode` and other camelCase fields); always uses hardcoded production URL. **Preserves the French handoff across redeploys**: passes `french_assistant_id` to `build_lease_config` from `manager_vapi_config.vapi_lease_french_assistant_id` (per-manager; select falls back gracefully if the column is absent) and from `VAPI_SHARED_LEASE_FRENCH_ASSISTANT_ID` (shared) — so re-running it does NOT strip the gated handoff tool. Run with the backend venv (`backend/.venv/Scripts/python.exe`)
...
- `provision_lease_french_handoff.py` — creates the dedicated **French-only** lease assistant (nova-3 `fr`, `FRENCH_TRANSCRIBER_CONFIG`) and wires a **gated handoff** tool onto the entry assistant so French callers are transferred to it by `assistantId` with `contextEngineeringPlan:"all"` (continues, not restarts). **No squad, no phone-number repoint** — the inbound number stays on the entry assistant; English path unchanged. Shared-agent **pilot** is the default; `--all --fleet-only --confirm-fleet` does the per-manager rollout (needs migration 031). `--rollback [--all --fleet-only]` undoes it (removes the handoff from entries, deletes the French assistants, clears the column). Re-running provisioning **patches existing French assistants in place** (id read from the column) — no duplicates. Supports `--dry-run`; run with the backend venv (`backend/.venv/Scripts/python.exe`, base python lacks `supabase`). **Current state: LIVE on all 4 per-manager agents (2026-06-26), in testing; shared agent not wired.** Routing = explicit English-first language gate (see "French handoff routing" under Voice & language config). See `docs/development_plans/PLAN_lease_agent_french_squad_routing_2026-06-26.md`
...
| Method | Path | Description |
|---|---|---|
| POST | `/flats/verify-phone` | **VAPI endpoint** — verifies caller phone matches tenant; returns `{status: valid/invalid/vacant}`. Optional `?language=en\|fr` (set per-assistant on the tool URL) locks `tenants.preferred_language` on first valid match (migration 032) |
...
| POST | `/voice/lease-eoc-webhook` | Lease EOC safety-net — builds a complete lead from `analysisPlan` structured data if `submit_lease_lead` never fired; bare `Unknown` partial only when no structured data |
| POST | `/voice/inbound-router` | **VAPI assistant-request** handler for the complaint number (its `server.url`; number has no static assistantId once routed). Returns `{assistantId}` per caller from `tenants.preferred_language`: `fr` → FR assistant + `assistantOverrides.firstMessage` (direct French opener), `en` → English-locked assistant, unknown/error → entry assistant (bilingual gate). Always HTTP 200 |
...
- Resolves `manager_id` **directly from the flat** — caller phone → `tenants.flat_uuid` → `flats.manager_id` (Step 8b), and on complaint creation the confirmed `flat_number` → `flats.manager_id` is authoritative (Step 10). `flats.manager_id` is NOT NULL on every flat incl. building-less standalone units (migration 029), so standalone-unit complaints get a real owner. **Replaced (2026-06-24)** the old flat → building → `properties_list.manager_id` chain, which returned NULL for standalone units (complaint lost to RLS) and was fragile when `properties_list.manager_id` was NULL. `submit_complaint` no longer takes `property_group_id` (the webhook never used it — complaints store no property group)
...
- Safety-net for the lease agent's end-of-call-report (analysisPlan)
- If `submit_lease_lead` already saved a lead for this `call_id`, skips (dedup)
- Otherwise reads `message.analysis.structuredData` (extracted by `analysisPlan.structuredDataPlan`) and inserts a **complete** lead: `caller_name`, `qualification_status`, `budget_max`, `move_in_timeline`, `occupants`, `qualifying_answers` (employment/landlord_aware/pets/unit_of_interest), best-effort `listing_uuid` resolved from spoken `unit_of_interest` (unique substring match on the manager's active listings). Fires the same qualified-lead notification as `/voice/lease-lead-direct`
- Only when no structured data is present does it fall back to the bare `caller_name="Unknown"` partial lead with transcript excerpt
...
- `complaint` → assistant chosen by `_complaint_assistant_for_phone` from `tenants.preferred_language` (`en` → English-locked assistant, `fr` → FR assistant + direct French opener, unknown → entry gate assistant `VAPI_COMPLAINT_ASSISTANT_ID`); caller ID `VAPI_COMPLAINT_NUMBER_ID` (`+14382567782`)
...
| Method | Path | Description |
|---|---|---|
| GET | `/leasing/find-units?query=&manager_id=` | **New** — query-first unit lookup; searches flat_number, title, address, building name, and property group name; returns `{found, count, units: [{listing_uuid, flat_number, building_name, property_name, address, bedrooms, bathrooms, floor_number, monthly_rent, available_from}], available_cities: [...], now}` — up to 5 matches; used by the new lease agent flow. **`now`** is the current Montréal (Eastern, DST-aware via `ZoneInfo("America/Toronto")`, fixed-offset fallback) date/time as `YYYY-MM-DDTHH:MM:SS`, returned on **every** path — the lease agent has no `Verify_phone_number` datetime, so this is its only real "today" for move-in vs `available_from` reasoning. **`available_cities`** is the distinct, display-cased set of cities across this manager's active listings, returned on **every** call (incl. no-match) so the agent can offer real alternatives instead of hallucinating. **Search algorithm:** token-based substring (query split on whitespace, tokens ≤ 2 chars ignored, ANY token in haystack matches) **plus** an accent-/spelling-tolerant city fallback via `app/services/city_matching.py` (`normalize_place` strips diacritics + hyphens, expands St/Ste→Saint/Sainte; `city_matches` does substring + FR/EN alias map + `difflib` fuzzy ratio ≥ 0.82) — handles Montréal/Mont-réal/Montreal, Saint-Léonard/St-Léonard, and phonetic near-misses. **Also returns a `disambiguation` block** `{needs_confirmation, field:"city", spoken, candidates:[{value, rent_low, rent_high}]}` — the banded "did you mean X or Y?" tier (`rank_candidates` in `city_matching.py`): `needs_confirmation` is true **only** when the spoken city is genuinely confusable with ≥1 real city (CONFIRM band); a clear winner auto-accepts (no extra turn) and junk falls through to `available_cities`. Each candidate carries its rent range so the prompt disambiguates homophones by rent/ordinal, never by re-saying a proper noun. **Gate unified with the ranker (fix):** the match gate previously used only `city_matches` (difflib ≥ 0.82), which is weaker than `rank_candidates`/`similarity` (adds rapidfuzz + Metaphone phonetic) — so a city the ranker was confident about could be **silently dropped** from `units` while no disambiguation was offered (caller heard "nothing found" for a city actually served, e.g. "San Lazaro" → Saint-Lazare). Now when `rank_candidates` returns **AUTO_ACCEPT**, that city is folded into the match gate so its units return directly; **CONFIRM** still drives the disambiguation question. `rank_candidates` is computed once and shared between the gate and `_build_city_disambiguation` so the two can never disagree |
...
**`/leasing/search-listings` (the lease agent's inventory-search tool; params `bedrooms`/`bathrooms`/`budget_max`/`city`/`state`/`parking`/`laundry`/`quebec_size`):** all non-numeric filters are Python post-fetch (after a 20-row `lte budget`/`ilike state` DB query, top 5 returned). **`quebec_size` is matched tolerantly, not by DB `eq`** — `_normalize_quebec_size` collapses `½`, `1/2`, and `.5` (with or without spaces) to the stored `<n>½` form on both sides before comparing, so a caller-stated "4 1/2" or "4.5" matches stored "4½". (Stored `quebec_size` is clean UTF-8 `<digit>+U+00BD`; the normalizer also strips a stray `Â` defensively.) The response also includes **`now`** (current Montréal date/time, same as `find-units`) so move-in reasoning at the search-results step has a real "today". **Bug fixed 2026-07-03 (Transcript3):** the SELECT didn't fetch `quebec_size`, so the post-fetch size filter compared against `None` and dropped **every** row — any search with a size returned `count: 0` and the agent told callers a 4½ that existed didn't (then flip-flopped when challenged, quoting the earlier `find_units` result). `quebec_size` is now in the SELECT of both `search-listings` and `listings-for-agent`, and `_format_listings` emits it in each listing (matching what the FR prompt already claimed). Regression tests in `backend/tests/integration/test_listings_for_agent_v2.py` assert the column is fetched and the tolerant match works end-to-end.
...
| POST | `/import/properties` | CSV or XLSX → PropertyGroup + Building + Flat hierarchy; optional `column_mapping` form field (JSON). **Only `flat_number` is a required column.** A row WITH `building_name` creates the property/building chain (requires `property_name`); a row WITHOUT `building_name` creates a **standalone unit** (`building_id` NULL, `manager_id = user["sub"]`). Every flat is inserted with `occupied: False` (mirrors `create_flat`) — required so vacant imported units are listable; without it they inherit a non-false DB default and `POST /leasing/listings` rejects them. If a row has `tenant_name` + `tenant_phone`, a tenant is created, the unit flipped to `occupied=True`, and an active rent added from `rent_amount` (`_maybe_create_tenant_and_rent`). Unit attrs pulled via `_flat_attrs_from_row` (floor/bed/bath/living_rooms/kitchen + structured address) |
...
- `VAPI_COMPLAINT_FRENCH_ASSISTANT_ID` — dedicated French-only assistant for the complaint agent's French handoff (set after running `provision_complaint_french_handoff.py`; consumed by it and by `update_shared_agents.py` to keep the handoff across redeploys). Also the direct route target for tenants with `preferred_language='fr'` (must be on Render)
- `VAPI_COMPLAINT_ENGLISH_ASSISTANT_ID` — English-locked complaint assistant for tenants with `preferred_language='en'` (created by `provision_complaint_language_routing.py`; must be on Render). Id: `43d28cbd-d01f-406f-bc70-646f45852a8a`
...
- `build_complaint_config(backend_url)` — global complaint agent (Option B, multi-group); complaint agent books **manager callbacks** (appointment `type="callback"`, `tenant_phone` set) — agent checks `check_availability` before scheduling, then calls `submit_complaint` with `appointment_date`. Like the lease agent, its tools carry NO static `request-start` filler `content` (all `messages` lists empty); the model speaks every "checking" phrase itself via the `[System-Check Phrases — Bilingual & Rotating]` block in `COMPLAINT_SYSTEM_PROMPT` (first check exact, subsequent rotated, language-matched — same spec as the lease agent below). **`submit_complaint` is a `function`-type tool (its own `server.url`=`/voice/webhook`, `async`, `strict`; required fields: `category`, `flat_number`, `appointment_date`, `description`).** It no longer takes `property_group_id` (2026-06-24) — the webhook never read it and resolves the owner from `flats.manager_id` instead; removing the unused required field also simplifies the model's tool call. **Prompt hardening (2026-06-23):** the model was narrating "complaint logged" without ever emitting `submit_complaint` (had all data — verified `property_group_id`, `available` slot — but treated the flow as speech). Mirroring the lease agent's hard tool-call rules, `COMPLAINT_SYSTEM_PROMPT` now (a) **excludes `submit_complaint` from the "say a checking phrase first" list** (that pre-narration instruction triggered the skip), (b) makes step 4/5 of `[CALLBACK SCHEDULING FLOW]` an explicit "you MUST emit the call; never say 'logged' without calling it", and (c) adds a `[Complaint Submission — NO EXCEPTIONS]` block ("submit_complaint is the ONLY way to file; a call without it has FAILED"). No tool/webhook/schema change — the `/voice/webhook` `submit_complaint` path is unchanged. **French handoff (2026-07-01, parity with the lease agent):** `build_complaint_config` takes an optional `french_assistant_id` arg (gated/additive — when null the English path is byte-identical to before). When set, the ENTRY assistant gains a `[French Routing — STRICT]` block, a `_complaint_handoff_tool` (`type:"handoff"`, destination `assistantId`, `contextEngineeringPlan:{type:"all"}`), and a static bilingual gate `first_message`; French callers are handed off to `build_complaint_config_french(backend_url)` — a FRENCH-ONLY complaint assistant on `FRENCH_TRANSCRIBER_CONFIG` (nova-3 `fr`) with a static French opener (`_COMPLAINT_FRENCH_FIRST_MESSAGE`) and a `[French Call — Continuation]` note, running the identical complaint/appointment flow (same tools, same `/voice/webhook`, no handoff tool — it is the target). Two shared French-speech rules were added to `COMPLAINT_SYSTEM_PROMPT` (so both entry + FR assistant inherit them; no-op on English calls): `[French Register — STRICT]` (formal **vous**, never **tu**) and `[Date & Time Pronunciation — STRICT]` (speak dates/years/clock times as French words — the ElevenLabs voice reads bare digits with an English pronunciation on French calls; e.g. "2026-02-09" → "le neuf février deux mille vingt-six", 15:00 → "quinze heures"; tool submissions keep ISO). Complaint agent is a single GLOBAL assistant, so unlike the per-manager lease case there is no DB column — the French assistant id lives in env `VAPI_COMPLAINT_FRENCH_ASSISTANT_ID`. Provisioned by `provision_complaint_french_handoff.py` (create/patch-in-place + entry wiring, `--dry-run`/`--rollback`); `update_shared_agents.py` passes `french_assistant_id` from that env var so redeploys don't strip the handoff. `_complaint_assistant_shell` factors the shared dict (mirrors `_lease_assistant_shell`)
- `build_complaint_config_french(backend_url)` — FRENCH-ONLY complaint assistant (handoff target); nova-3 `fr` transcriber, static French opener, continuation note; identical flow otherwise
...
- **④ Handoff:** agent calls `submit_lease_lead` EXACTLY ONCE with full lead data (`qualification_status` = qualified / not_qualified / unmatched). No-match path still submits an `unmatched` lead. The prompt makes this a hard rule — `submit_lease_lead` is the ONLY tool that records a lead; the agent must never call `find_units`/`search_listings` to wrap up (regression fixed 2026-06-23 — see lead-capture safety net below)
- **Lead capture — two layers (2026-06-23):** (1) the agent calls `submit_lease_lead` → `/voice/lease-lead-direct` during the call; (2) a deterministic **safety net** — `analysisPlan.structuredDataPlan` (`_LEASE_STRUCTURED_DATA_PLAN` in `_lease_assistant_shell`) has VAPI extract the lead fields (caller_name, qualification_status, budget_max, move_in_timeline, occupants, employment, landlord_aware, pets, unit_of_interest, notes) from the transcript at end-of-call and post them in the end-of-call-report. `/voice/lease-eoc-webhook` reads `message.analysis.structuredData` and inserts a COMPLETE lead even if `submit_lease_lead` never fired; it only falls back to the bare `caller_name="Unknown"` row when no structured data is present. Dedup by `call_id` means whichever path fires first wins. Transcripts carry no UUIDs, so the webhook resolves `unit_of_interest` text → `listing_uuid` best-effort (unique substring match on the manager's active listings' street_address/city/flat_number). **Why this exists:** between 2026-06-17 and 06-23 `submit_lease_lead` never fired — **root cause (fixed 2026-06-23): all lease apiRequest tools shared `function.name="api_request_tool"`, so VAPI/OpenAI deduped them and only the first (`find_units`) was reachable**; the model looped on `find_units` and every lead became an `Unknown` EOC fallback. Fixed by `_alias_apirequest_tool_names()` (unique `function.name` per tool — see §6 "VAPI tool naming"). The structured plan remains as a belt-and-suspenders guarantee of a real lead regardless of model tool-calling
...
- **French register lock** (`[French Register — STRICT]` block in `_LEASE_SYSTEM_PROMPT_BASE`, added 2026-06-25): on French calls the agent must address the caller with formal **vous** for the entire call and never use **tu**/`t'`/tu-conjugations, even if the caller uses tu. The "didn't catch that" recovery line is fixed as "Je n'ai pas bien compris, pourriez-vous répéter?" (replacing the broken "Tu n'es pas bien compris" seen in 06-22 transcripts). The Q2 landlord-awareness step also carries an explicit French (vous) template — "Est-ce que votre propriétaire actuel sait que vous cherchez à déménager?" — so the model stops mistranslating it on the fly. Prompt-only; lease agent only; English path untouched (R3 in `PLAN_lease_agent_french_refinement_options_2026-06-25.md`)
- Never say "let me put you through to the team" — always "I'll make sure someone reaches out"
- **Background tool calls / silence prevention (reverted to model-spoken, locked-language 2026-06-23):** `find_units` and `search_listings` carry **no** static `request-start` `content` (all `messages` lists empty), matching the complaint agent. **Why the static bilingual string was removed:** a single static `content` cannot respect the per-call language lock — it spoke EN+FR on every lookup even on an English-locked call, and (combined with the tool-name loop bug) repeated on every retry. The model now speaks a short "checking" phrase itself in the caller's locked language. `submit_lease_lead` keeps no filler (the model speaks the closing line as it fires, `async: True`). `find_units` stays `async: False`.
- **System-check phrases — model-spoken, locked language** (`[System-Check Phrases — Locked Language Only]` block in `_LEASE_SYSTEM_PROMPT_BASE`): the model says a short, varied "one moment" phrase in the caller's locked language the instant it fires `find_units`/`search_listings` (never both languages, never a slash, never an appended translation), then keeps the flow moving; no checking phrase for `submit_lease_lead`. Mirrors the complaint agent's spec.

**Model:** provider `"openai"`, `gpt-5.2-chat-latest` for **all** agents (complaint/tenant `build_assistant_config`/`build_complaint_config`, and the lease agent `_lease_assistant_shell`). The lease agent was briefly on `gpt-4o` for lower time-to-first-token (latency experiment 2026-06-23) but **reverted to `gpt-5.2-chat-latest` 2026-06-23** — gpt-4o failed to follow the multi-step tool routing (used `find_units` for unit size and never reached `search_listings`), breaking unit discovery. Latency must be addressed without downgrading the model. The `eleven_flash_v2_5` TTS latency fix was kept.
...
- **French handoff routing (lease agent, 2026-06-26):** because the STT language is fixed at call-start and cannot switch mid-call, French callers are routed to a **dedicated French-only assistant** (`FRENCH_TRANSCRIBER_CONFIG` — Deepgram nova-3 **`language:"fr"`**, threshold 0.4; better French WER than `multi` and supports keyterm prompting). **Québec-French keyterm biasing (2026-07-07):** `FRENCH_TRANSCRIBER_CONFIG` now ships a static `keyterm` list seeded from `QUEBEC_FRENCH_KEYTERMS` — ~17 distinctive Québécois vocab/anglicism/booking terms (`pantoute`, `astheure`, `faque`, `tantôt`, `canceller`, `une job`, `plate`, `souper`, `ça adonne-tu`, …) that France-French-trained nova-3 tends to garble. Curated small on purpose (~500-token keyterm budget; over-long lists cause false insertions). Ultra-common contractions and acoustic pronunciation features are intentionally excluded (keyterms can't fix those). **Per-manager place-name layer (2026-07-07):** `build_lease_config_french(..., place_terms=[...])` merges each manager's **real cities + street names** on top of the base Québécois terms via `build_french_transcriber()` → `build_french_keyterms()` (dedup case-insensitive, capped at `_MAX_FRENCH_KEYTERMS=200`; base terms never mutated). `provision_lease_french_handoff.py._fetch_place_keyterms()` sources them from that manager's ACTIVE `lease_listings` (distinct `city` + `street_address`, house numbers stripped conservatively by `_street_name()` — Québec ordinal streets like "8e Avenue" preserved). Complaint FR agent is global (no listings) so it keeps only the base terms. Keyterms bake into the assistant at provision time → **re-run `provision_lease_french_handoff.py` to refresh** when a manager's listings change (`update_lease_agents.py` does NOT rebuild FR assistants, so it won't clobber them; new managers get keyterm-enabled FR agents on the next fleet-script run — the only path that builds FR agents). **Provisioned LIVE to all 4 per-manager FR assistants on 2026-07-07** (`provision_lease_french_handoff.py --all --fleet-only --confirm-fleet`); verified by GET on `8900a5c4…` → `nova-3 fr`, 21 keyterms (17 base + Saint-Lazare / Rue Du Comtois / Châteauguay / Boulevard Sainte-Marguerite). Source: `quebec_french_glossary.xlsx`. The entry assistant keeps `multi`; when built with `french_assistant_id` it gains a `[French Routing — STRICT]` block + a Vapi **handoff** tool (`type:"handoff"`, destination `assistantId`, `contextEngineeringPlan:{type:"all"}`). Routing is an **explicit English-first language gate** (2026-06-26): the entry opens with a short English intro and asks "Would you like to continue in English, or in French? / Préférez-vous continuer en anglais ou en français?" (the one permitted bilingual line), does no leasing until the caller picks, then hands off on **anything that is not a clear English answer** — French, a French answer, OR a garbled / non-Latin-script transcription (Devanagari/Cyrillic/CJK); English continues on the entry assistant only on a clear English reply. **Hardened 2026-06-29 (`not-clearly-English → French`):** previously it routed only on an explicit French choice, so when the gate's `multi` transcriber mis-identified a native French speaker's short answer as **Hindi** (emitted `आप कहां से?` in Devanagari) the gate never saw "French", fell into the "unclear → ask again" branch, and **dead-looped** — never reaching the working `language:"fr"` agent. Since French is the only non-English route, any non-English/garbled/non-Latin output is now treated as proof the caller isn't speaking English and hands off; the single re-ask is reserved for true silence only. See `docs/diagnoses/lease_agent_french_hindi_misid_language_gate_2026-06-29.md`. No squad, no phone-number change. Builders: `build_lease_config_french[_shared]()` (target) and the optional `french_assistant_id` arg on `build_lease_config[_shared]()` (entry, gated/additive — English path byte-identical when null). Provisioned by `provision_lease_french_handoff.py`. The French assistant carries a `[French Call — Continuation]` note so it continues mid-call rather than re-greeting. **French opener is a fixed `first_message` (2026-06-27):** the French assistant uses `first_message_mode:"assistant-speaks-first"` with a static `_LEASE_FRENCH_FIRST_MESSAGE` ("Parfait, je continue en français. Qu'est-ce que vous cherchez comme logement?") instead of a model-generated one. **Why:** with model-generated openers the destination assistant non-deterministically role-played the router ("je vous mets en relation avec mon collègue francophone…") off the carried context, then yielded the floor — leaving 6 s of post-handoff dead air (worked on one test call, failed the next). The static opener removes the generation latency, guarantees the greet-and-ask line, and ends on a question so the caller is prompted; the continuation note now explicitly forbids announcing any transfer/"collègue". `firstMessage` is sent to VAPI by both `_to_vapi_payload` mappers (`provision_lease_french_handoff.py`, `update_lease_agents.py`) and is scoped to the French builders only (English entry agents set no `first_message`, so it's filtered out and they keep their model-generated bilingual greeting). **The static-opener fix was provisioned LIVE to all 4 per-manager FR assistants on 2026-06-29** (`provision_lease_french_handoff.py --all --fleet-only --confirm-fleet`) and verified by GET on the FR assistant from the bug transcript (`8900a5c4…`: `firstMessageMode:"assistant-speaks-first"`, `firstMessage` = the static line, transcriber `nova-3 fr`); a live French test call to confirm no dead-air is the remaining validation step. The handoff routing itself is **LIVE on all 4 per-manager agents (since 2026-06-26); the shared agent is not yet wired.** Per-manager French assistant ids live in `manager_vapi_config.vapi_lease_french_assistant_id`. Rollback: `provision_lease_french_handoff.py --rollback --all --fleet-only`.
- **French handoff routing (complaint agent, 2026-07-01):** same architecture as the lease agent above, applied to the single GLOBAL complaint agent. The entry complaint assistant keeps `COMPLAINT_TRANSCRIBER_CONFIG` (nova-3 `multi`, threshold 0.6); when built with `french_assistant_id` it gains a `[French Routing — STRICT]` block + a `type:"handoff"` tool (`contextEngineeringPlan:{type:"all"}`) and opens with a **static bilingual gate** `first_message` ("…Would you like to continue in English, or in French? … Préférez-vous continuer en anglais ou en français?") kept on `assistant-speaks-first` (deterministic, no LLM latency on the routing turn; matches the complaint agent's existing static-opener design — this is the one intentional deviation from the lease entry, which uses a model-generated gate). It does no maintenance/verification until the caller picks, then hands off on **anything that is not a clear English answer** — French, a French reply, or a garbled / non-Latin-script transcription (Devanagari/Cyrillic/CJK); the single re-ask is reserved for true silence only (same `not-clearly-English → French` hardening as the lease gate). The FR complaint assistant (`build_complaint_config_french`) runs `language:"fr"`, a static French opener, and a `[French Call — Continuation]` note that forbids announcing any transfer/"collègue"; it conducts the whole call in French. No squad, no phone-number change. Builders: `build_complaint_config_french()` (target) + optional `french_assistant_id` on `build_complaint_config()` (entry, gated/additive — English path unchanged when null). Provisioned by `provision_complaint_french_handoff.py`; FR assistant id in env `VAPI_COMPLAINT_FRENCH_ASSISTANT_ID`. **Status: code landed 2026-07-01, not yet provisioned/live** — run `provision_complaint_french_handoff.py --dry-run` then without `--dry-run`, add the printed env var, and validate with EN + FR test calls. Rollback: `provision_complaint_french_handoff.py --rollback`.
...
- **Complaint language routing (2026-07-02):** THREE complaint assistants — entry (bilingual gate + FR handoff, unknown callers), English-locked (`VAPI_COMPLAINT_ENGLISH_ASSISTANT_ID`), French (`VAPI_COMPLAINT_FRENCH_ASSISTANT_ID`). A tenant's first valid `verify-phone` locks `tenants.preferred_language` (tool URL carries `&language=en|fr`); afterwards inbound is routed by `/voice/inbound-router` (number's `server.url`, no static assistantId once `provision_complaint_language_routing.py --route-number` has run) and outbound by `_complaint_assistant_for_phone`. Deploy order + rollback: `docs/development_plans/PLAN_complaint_agent_language_preference_routing_2026-07-02.md`
- **Inbound needs the number to resolve to an assistant in VAPI** — either a static `assistantId` binding or a `server.url` that answers the per-call `assistant-request`. A number with NEITHER silently kills only inbound calls (endedReason `call.in-progress.twilio-completed-call`, empty assistantId in the call log — outbound still works since it passes the assistant per-call); the complaint line failed this way on 2026-07-02. **Current state:** the complaint number uses `server.url = /voice/inbound-router` with `assistantId=null` (per-tenant language routing) — do NOT "fix" the null assistantId by re-binding it statically; that would disable language routing. Rollback, if ever needed: `provision_complaint_language_routing.py --rollback`.
...
- **Live phone numbers** (do not reassign):
  - `+14382567782` → complaint agent (`VAPI_COMPLAINT_NUMBER_ID`); **no static assistantId by design** — inbound resolves per-caller via `server.url = /voice/inbound-router` (language routing, 2026-07-02)
  - `+14313404212` → leadpipecrm manager `28c43c77` (active)
  - `+14313415768` → available in pool (unassigned)
- Outbound complaint call routes by `tenants.preferred_language` via `_complaint_assistant_for_phone` (entry gate when unknown); caller ID is always `VAPI_COMPLAINT_NUMBER_ID`.
