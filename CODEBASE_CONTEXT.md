...
- **Background tool calls / silence prevention (changed 2026-06-23)** — `find_units` and `search_listings` now carry a deterministic `request-start` message with bilingual `content` (`"Let me check that in my system, one moment. Un instant, je vérifie ça."` / `"Let me take a quick look at that for you. Un instant, je regarde ça."`, `blocking: False`). VAPI speaks it the instant the tool fires, so a blocking `find_units` can no longer produce dead air. **Why the switch from model-spoken phrases:** relying on the model to speak a "checking" phrase before a blocking call was unreliable — on a live call the model fired `find_units` first and left ~17s of silence. A request-start `content` plays automatically; per VAPI this is more reliable than LLM generation. `submit_lease_lead` keeps no filler (the model speaks the closing line as it fires, `async: True`). `find_units` stays `async: False`.
- **System-check phrases — now automatic** (`[System-Check Phrases — handled automatically]` block in `_LEASE_SYSTEM_PROMPT_BASE`): the prompt instructs the model NOT to announce lookups itself (the request-start filler does it) and to keep the conversation flowing instead — avoiding double-speak. (Replaced the prior bilingual rotating model-spoken phrase list.) Tradeoff: the filler is a single bilingual static string rather than a varied, language-locked phrase, in exchange for guaranteed no-dead-air coverage.
