...
- `update_shared_agents.py` — push latest config to complaint agent + shared lease agent; uses direct **HTTP PATCH** (same as `update_lease_agents.py`; rewritten off the VAPI SDK, which silently dropped `firstMessageMode` and apiRequest tool subfields). Supports `--dry-run`
